...
namespace Reversi.DataStructures
{
    ...
    public static class IGridExtensions
    {
        ...
        /// <summary>
        /// Takes a slice out of the grid.
        /// </summary>
        /// <typeparam name="T">Type of the elements of the grid.</typeparam>
        /// <param name="grid">Grid.</param>
        /// <param name="start">Starting position.</param>
        /// <param name="direction">Slice direction.</param>
        /// <returns>A sequence representing the slice. At index 0, the sequence contains
        /// the item at position <paramref name="start"/>. At index 1, the sequence contains
        /// the item at position <paramref name="start"/> + <paramref name="direction"/>. Negative indices are allowed,
        /// as long as they keep referring to positions within the grid boundaries.</returns>
        ...
        /// <summary>
        /// Extracts a row from the grid. This sequence is not a copy
        /// of the row, but instead keeps a reference to the grid
        /// and remains synchronized with its contents. I.e. changes
        /// to the grid propagate to the row sequence.
        /// </summary>
        /// <typeparam name="T">Type of the elements of the grid.</typeparam>
        /// <param name="grid">Grid.</param>
        /// <param name="row">Row.</param>
        /// <returns>Sequence containing the elements of the row.</returns>
        ...
        /// <typeparam name="T">Type of the elements of the grid.</typeparam>
        /// <param name="grid">Grid.</param>
        /// <returns>All rows of the grid.</returns>
        ...
        /// <summary>
        /// Extracts a column from the grid. This sequence is not a copy
        /// of the column, but instead keeps a reference to the grid
        /// and remains synchronized with its contents. I.e. changes
        /// to the grid propagate to the column sequence.
        /// </summary>
        /// <typeparam name="T">Type of the elements of the grid.</typeparam>
        /// <param name="grid">Grid.</param>
        /// <param name="column">Column.</param>
        /// <returns>Sequence containing the elements of the column.</returns>
        ...
        /// <typeparam name="T">Type of the elements of the grid.</typeparam>
        /// <param name="grid">Grid.</param>
        /// <returns>All columns of the grid.</returns>
        ...
        public static int Count<T>(this IGrid<T> grid, T value)
        {
            return grid.Count( x => x == null ? value == null : x.Equals( value ) );
        }

        public static bool InCorner<T>(this IGrid<T> grid, Vector2D position)
        {
            return ( position.X == 0 || position.X == grid.Width - 1 ) && ( position.Y == 0 || position.Y == grid.Height - 1 );
        }

        public static bool OnRim<T>(this IGrid<T> grid, Vector2D position)
        {
            return position.X == 0 || position.Y == 0 || position.X == grid.Width - 1 || position.Y == grid.Height - 1;
        }
    }
    ...
    public class Grid<T> : GridBase<T>
    {
        ...
        public Grid( T[][] xss )
            : this( xss[0].Length, xss.Length, p => xss[p.Y][p.X] )
        {
            // NOP
        }
