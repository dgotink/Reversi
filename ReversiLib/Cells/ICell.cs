...
namespace Reversi.Cells
{
    ...
    public static class ICellExtensions
    {
        ...
        public static ICell<R> Select<T, R>(this ICell<T> cell, Func<T, R> function)
        {
            return Cell.Derived<T, R>( cell, function );
        }

        public static ICell<R> SelectMany<T1, T2, R>(this ICell<T1> x, Func<T1, ICell<T2>> fy, Func<T1, T2, R> function)
        {
            var c1 = x;
            var c2 = fy( c1.Value );

            return Cell.Derived( c1, c2, function );
        }
    }
}
